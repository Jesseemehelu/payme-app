...
app.post('/api/luck-mining/upgrade', requireLoginMiningUpgrade, async (req, res) => {
  try {
    ...
    const result = await withLuckMiningLock(req.user.id, async () => {
      ...
      const requiredReferrals = Number(target.referrals) || 0;
      const currentReferrals = number(user.totalReferrals);
...
app.post('/api/auth/localtest-signup', async (req, res) => {
  try {
    ...
    if (!user) {
      if (!selectedLanguage) {
        return res.status(400).json({ success:false, needsLanguage:true, message:'Please choose your language before continuing.' });
      }
      const newUser = {
        id: generateUserId(),
...
async function processReferral(referrer, newUser) {
  ...
  /* Referrals no longer award Gems. referralEarnings remains unchanged. */
  await updateUser(referrer);
  return confirmedAt;
}

async function processPendingReferralAfterJoin(newUser) {
  try {
    if (!newUser || !newUser.referredBy) return false;
    const daily = normalizeDailyReward(newUser);
    if (daily.referralConfirmedAt) return false;
    const code = String(newUser.referredBy || '').trim().toUpperCase();
    ...
    daily.referralConfirmedAt = new Date().toISOString();
    newUser.dailyReward = daily;
    await updateUser(newUser);
    return true;
  } catch (err) {
    console.error('Pending referral confirmation error:', err);
    return false;
  }
}
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      // --------------------------------------------------
      // SECURITY: req.headers.host is fully attacker-controlled (it's just the
      // Host header the client sends) — trusting it here meant anyone hitting
      // this endpoint through a public tunnel with `Host: localhost` could
      // skip Telegram verification entirely and mint arbitrary accounts/sessions.
      // Require the connection to genuinely come from loopback instead, and
      // never allow the bypass at all once NODE_ENV=production is set.
      const remoteAddr = String(req.socket?.remoteAddress || '');
      const isLoopback = remoteAddr === '127.0.0.1' || remoteAddr === '::1' || remoteAddr === '::ffff:127.0.0.1';
      const isLocal = !isProduction && isLoopback;
      ...
      // In production, referrals can ONLY originate from Telegram's
      // verified start_param (the referral link). Ignore arbitrary referral
      // codes posted by the browser.
      const cleanRefInput =
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (isLocal) {
        ...
        await processPendingReferralAfterJoin(user);
        ...
        return res.json({
          ...
          referralConfirmed: !!user.dailyReward?.referralConfirmedAt
        });
